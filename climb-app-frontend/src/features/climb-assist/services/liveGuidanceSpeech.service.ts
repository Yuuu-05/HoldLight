...
function holdTargetLabelZh(hold: Hold | null) {
  if (!hold) return '目标点';
  return `${holdDescriptorZh(hold, hold.role === 'foot' ? 'leftFoot' : 'leftHand')}${hold.label ? ` ${hold.label}` : ''}`;
}
...
export function buildGuidanceCueSpeechZh({
  ...
  const prefix = `第${cueIndex + 1}步，共${totalCues}步。`;
  ...
  if (cueIndex === 0 || targetHold.role === 'start') {
    return `${prefix}先建立起始姿势，双手上到${targetLabel}，保持三点接触，稳住再继续。`;
  }

  if (targetHold.role === 'finish') {
    return `${prefix}${limbLabel}去找${targetLabel}。抓住终点以后先稳住身体。`;
  }

  if (isFootLimb(limb) || targetHold.role === 'foot') {
    return `${prefix}${limbLabel}去找${targetLabel}。脚先踩稳，再带动上半身。`;
  }

  return `${prefix}${limbLabel}去找${targetLabel}。动作放慢一点，先稳再伸。`;
}
...
export function buildLivePositionSpeechZh({
  ...
  if (!activeAnchor || distancePct === null) {
    return {
      speechText: `${limbLabel}没有清楚进入画面，请把它重新放回镜头里。`,
  ...
  if (distancePct <= targetThreshold * 0.72) {
    return {
      speechText: `${limbLabel}到了，稳住在${targetHold.label}上，先不要急着下一下。`,
  ...
  if (distancePct <= targetThreshold * 1.3) {
    return {
      speechText:
        correction.length > 0
          ? `${limbLabel}，${correction.join('，')}。去${targetHold.label}。`
          : `${limbLabel}，先稳住，目标就在${targetHold.label}附近。`,
...
export function buildTargetReachedSpeechZh({
  ...
  return `当前目标已到。${buildGuidanceCueSpeechZh({

...
const reviewStateLabelsZh: Record<RouteReviewState, string> = {
  'auto-approved': '语义检查通过',
  'review-recommended': '建议人工复核',
};

const reviewStateLabelsEn: Record<RouteReviewState, string> = {
  'auto-approved': 'Semantic checks passed',
  'review-recommended': 'Setter review recommended',
};
...
export function formatHoldTarget(hold: Hold | null | undefined, language: Language) {
  if (!hold) return language === 'zh' ? '暂无目标点' : 'No active target yet';
  const color = formatHoldColor(hold.color, language);
  return language === 'zh' ? `${hold.label}（${color}）` : `${hold.label} (${hold.color})`;
}
